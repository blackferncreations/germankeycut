...
An interface is a point at which an offer is encountered, interpreted, evaluated, recommended, delivered or carried forward.

Different interfaces perform different functions.
...
This does not mean that less information is always better.

The appropriate information density depends on what the interface has to accomplish.

A public positioning page may require substantial reduction.
...
Not every piece of information has equal importance at every point in an evaluation.

Some information must appear first.

Some becomes useful only after a prior condition has been established.

Some belongs in another interface entirely.

The objective is therefore not maximum information availability at every point.
...
None of this is a claim that the interface is well executed. It is a record of what was decided and why. The reasoning is documented here; the implementation is visible at [germankeycut.com](https://germankeycut.com/) and can be measured there.
...
These interfaces can use different information densities without becoming structurally inconsistent.

Consistency does not require identical communication.
...
The distinction between public and non-public does not determine importance.

A private proposal can have greater structural impact than a public campaign.

A handover can affect positioning.

A responsibility structure can affect positioning.

A delayed technical answer can affect positioning.

The market does not experience only what an organization publishes.
...
It can clarify hierarchy, expose evidence, establish terminology and support progression.

But communication remains connected to what occurs around it.

A strong public position followed by an inconsistent sales process creates contradiction.

A precise claim unsupported by evidence creates contradiction.

A specialist website followed by generalist execution creates contradiction.

The problem in these cases does not exist only within the message.

It exists between interfaces.
...
A technical buyer may require documentation.

A procurement process may require certification.

A partner may require evidence of specialization.

A client may evaluate repeated delivery.

A public visitor may not require the same information at the initial interface.

Public proof and structural evidence therefore do not have to appear at the same point.
...
Not every visitor requires the same next step.
...
They need to remain structurally coherent.
